...
- **Move / rotate / scale** — a Blender-style toolbar with a gizmo on the selected node (or a whole imported file), `G` / `R` / `S` mode keys, Local / Global orientation, editable position / rotation / scale fields, undo / redo, and a one-click reset; bones get the same gizmo for posing
- **Save menu** — one place to save a byte-for-byte copy of the original, export the whole scene or the selected node with its current transforms and poses to GLB / glTF / OBJ / STL / PLY / USDZ (defaulting to the source format when three.js can write it), or snapshot the view
