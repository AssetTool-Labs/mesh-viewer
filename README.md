...
- **Move / rotate / scale** — a gizmo on the selected node (or a whole imported file) with `G` / `R` / `S` mode keys, editable position / rotation / scale fields, undo, and a one-click reset; bones get the same gizmo for posing
