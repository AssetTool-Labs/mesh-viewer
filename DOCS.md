...
- **Save menu** — the download button in the sidebar header (next to `+ Import Mesh…`) opens one menu holding every way to write something to disk, so a byte-for-byte copy is never confused with a re-encoded export:
  - **Save a copy of the original file…** writes the opened file's bytes, unconverted, wherever you pick. This is also how you extract a real file from a virtual document (e.g. the left pane of a git diff), and the only way to save a format three.js cannot write.
  - **Export scene as…** writes every imported file, with current transforms and poses, to one new file — see *Export as a new file* below.
  - **Export selection as…** does the same for the selected node and everything under it (disabled with nothing selected).
  - **Save snapshot as PNG…** and the transparent-background variant capture the viewport — see *Snapshot* below.
- **Reveal the file** — the folder button next to the Save menu reveals the file in the Explorer view when it's inside the workspace, or in Finder / File Explorer when it isn't; it's hidden for virtual documents, which have nothing on disk to reveal.
...
- **Texture browser** — the Textures tab shows each texture in a **zoomable viewport** over a checkerboard: scroll to zoom about the cursor (past 1:1 you see the individual texels), drag to pan, double-click to refit; a corner readout shows the zoom. The optional UV-map overlay is drawn in the same view with a constant 1px stroke at any zoom, and the metadata below lists size, wrap, filter, color space, and which materials use it. The **⤢** button in the card header opens a fixed-size enlarged copy in a modal (`Esc` or click to dismiss). Compressed **KTX2/Basis** textures work too — being GPU-only, they're read back from a render target rather than drawn directly.
...
- **Texture-map toggles** — in **Material** or **Rendered** shading, a foldable **Shading options** group in the View tab exposes mode-specific controls; for these modes it holds **Texture maps** checkboxes that switch each PBR map on or off *in the lit render*: **Base Color**, **Normal**, **Metalness**, **Roughness**, **AO**, **Emissive**. Uncheck one to see the render without it — drop the normal map to judge the underlying geometry, or AO to see it unshadowed — and combine any subset. The group appears only in Material/Rendered, the modes that actually use the asset's maps; this is distinct from the **Textures** tab, which previews the individual 2D images.
- **UV ↔ 3D element selection** — in the Textures tab, the **Select** switch (Off / Vert / Edge / Face, keys `1`/`2`/`3`) turns on Blender-style element inspection: one selection, mirrored live in both the UV viewport and the 3D viewport. Hover an element in either view and it lights up in both; click to select, `Shift`+click to extend or remove. Ids are **seam-aware** — selecting a vertex on a UV seam lights every copy of it across islands, and a seam edge lights both of its UV copies, which makes the unwrap's cuts visible at a glance. Area tools: with the **Box** or **Lasso** tool active, **drag in the UV view** (Alt/middle-drag pans instead), or **`Shift`+drag in the 3D viewport** (`B` arms the next plain drag as a replacing select; plain drag always orbits). 3D area select follows the **X-ray** display toggle: with X-ray off only elements the camera can actually see are taken (decided by an ID pass — the front-most triangle at every pixel of the region); with X-ray on the region selects straight through, back faces and occluded parts included. **Double-click** selects the connected patch — the **UV island** when done in the UV view, the **3D connected component** in the viewport (on seam-heavy meshes these differ; that difference is the point). `A` toggles all/none, `Esc` cancels an in-flight marquee then clears, and `F` (or the **Frame highlight** button) localizes the highlight: over the UV view it flies the 3D camera to the region, over the viewport it zooms the UV view to the matching UV bounds, elsewhere it does both. The selection follows one mesh at a time — clicking another mesh in the 3D view switches the active mesh and the texture card with it (hover only tracks the active mesh). Selection is session-local; element modes suspend object picking until set back to Off. Highlights follow a skinned mesh's transform but not its live deformation, and skinned meshes area-select at their bind pose and always through (the visibility pass needs a stable surface).
- **Isolate a texture on the model** — in the Textures tab, **Isolate on model** shows the selected texture's channel *unlit* on the geometry — **Base Color**, **Normal Map**, **Metalness**, **Roughness**, **AO**, or **Emissive** — so a swapped, missing, or mis-authored map is obvious at a glance; materials without that map show their scalar factor as a flat color, and packed maps (e.g. a single ORM texture) show all their components. It is the on-geometry counterpart of the 2D preview above it. Like the skin-weight display it is session-local and overrides the shading dropdown without changing it; click the button again (**Stop isolating**), the chip's ✕, or any shading mode to return. The texture-map toggles are inactive while isolating.
- **Viewport status chip** — whenever the viewport shows a special view — an isolated texture channel, or the skin-weight display (which overrides the shading dropdown on skinned meshes) — a chip under the shading strip names it (e.g. *Inspect: Roughness*, *Skin weights: Isolate bone*) with a ✕ that returns to normal shading or turns weights off. Skin weights take precedence when both are active.
...
- **Blendshape controls** — when a scene has morph targets (blendshapes), a **Blendshapes** tab appears with a 0–1 slider per shape, named from the asset (falling back to `Morph N`). Meshes with morphs are grouped by name; a **Combine by name** toggle merges same-named shapes across meshes into one slider (useful for multi-submesh ARKit face rigs). **Reset all** zeroes every shape. When a clip animates the morph weights, the sliders track the live values as it plays; drag one to pose it by hand while paused. The tab stays hidden for assets without blendshapes.
- **Pose joints** — select a bone in the hierarchy or click a joint on the skeleton overlay to highlight it, then **Rotate** in the toolbar (or `R`) attaches a local-space rotation gizmo; the inspector also exposes editable Euler degrees (in that bone's existing rotation order). Child bones follow (FK): rotating an upper-leg joint swings the lower leg with it. Posing pauses the clip so the mixer does not overwrite the edit. Play or scrub still drives the clip and overwrites the pose. **Reset Pose** restores the bind pose snapshotted at load. `R` rotates the selected joint around the view axis by sweeping the mouse around it (Blender's default R); `R` again switches to trackball (mouse X/Y tumble). `X`/`Y`/`Z` lock a local axis; LMB/Enter confirms, Esc cancels. `Cmd/Ctrl+Z` / `Cmd/Ctrl+Shift+Z` undo and redo pose edits. IK and export are out of scope.
- **Transform nodes** — select any node in the hierarchy (a mesh, a group, or the top-level row for a whole imported file) to highlight it. A Blender-style toolbar on the viewport's left edge starts in **Select**, where a click only highlights and no gizmo appears; **Move / Rotate / Scale** (or press `G` / `R` / `S`) attach the gizmo and stay on until you pick Select again. It also holds the **Local / World** orientation toggle (Blender's *Local* / *Global*: the gizmo's axes follow the node's own frame or the scene's), **Undo / Redo**, and **Reset** (the same per-node reset as the inspector: the load-time transform for a node, or the whole bind pose when a bone is selected). As in Blender, an arrow moves along one axis, a square across two, and the center handle moves freely in screen space. The inspector's **Position**, **Rotation** (degrees, in the node's Euler order), and **Scale** fields are editable and track the gizmo live. `Cmd/Ctrl+Z` / `Cmd/Ctrl+Shift+Z` share one undo history with bone posing, and **Reset Transform** returns the node to its load-time values. Transforms are session-only and are not written back to the opened file (use *Export scene as…* or *Export selection as…* to keep them); changing the up axis re-seats the whole scene on the grid as before.
- **Export as a new file** — *Export selection as…* in the Save menu (or **Save Selection As…** in the Selection panel) writes the selected node and everything under it, with its current transform and pose baked in, to a new file; select a file's top-level row to save that whole model. *Export scene as…* writes every imported file at once, so a scene assembled with *Import Mesh…* comes out as one file with every file's animation clips. The dialog defaults to the source format when three.js can write it, and to GLB otherwise (always GLB for a scene built from several files); the extension you pick decides the format:

  | Format | Keeps |
  | --- | --- |
  | `.glb`, `.gltf` | geometry, materials, textures, skinning, the file's animations |
  | `.usdz` | geometry, PBR materials, textures |
  | `.obj` | geometry only (no materials, skinning, or animation) |
  | `.stl`, `.ply` | geometry only, merged into one mesh |

  FBX, DAE, 3DS, 3MF, VRML, STEP, LWO, VOX, PCD, XYZ, and KMZ sources have no writer and default to GLB. Gaussian splats cannot be exported.
- **View helpers** — grid, axes, bounding box, auto-rotate, IBL studio/neutral environment, background color picker.
- **Snapshot** — the camera button in the shading HUD, the Save menu, or the *3D Mesh Viewer: Save Snapshot as PNG* command exports the current view to a PNG via a Save dialog. The scene is captured as shown — grid and axes are included if visible, the corner nav gizmo never is — so toggle overlays off first for a clean shot, or set the background color you want. **Hold Alt/Option** while clicking (or run *Save Snapshot as PNG (Transparent Background)*) to export with a transparent background instead of the scene background.
...
| Move / rotate / scale selection | drag the gizmo | `G` / `R` / `S` pick the gizmo mode |
| Undo / redo a transform or pose edit | — | `Cmd/Ctrl+Z` / `Cmd/Ctrl+Shift+Z` |
...
- Node transforms (move / rotate / scale) and bone poses are session-only viewer state: nothing is written back to the opened file, and re-opening it starts from the authored transforms. *Export scene as…* / *Export selection as…* write a new file instead. There is no snapping, pivot editing, or multi-select.
- *Export selection as…* writes the node relative to its file's own frame, and *Export scene as…* writes every file relative to the scene: edits on the nodes and their parents are baked in, the viewer's up-axis display rotation and grid seating are not. A skinned mesh whose skeleton sits outside the selected node cannot be exported on its own (the writers would emit an invalid skin or drop it); the Save menu names the ancestor to select instead, usually the armature or the file's top-level row. Selecting a node below a file's top-level row exports it without that file's animation clips. The file always carries the asset's own materials: the shading mode, texture inspection, skin-weight display, X-ray, map toggles, and overlays never affect it. Hidden nodes are skipped by the glTF and USDZ writers; OBJ, STL, and PLY write them too. Compressed KTX2 textures are decoded through the GPU before being embedded as PNG, so they grow.
...
- Blendshape sliders clamp to the 0–1 range (the ARKit/glTF convention); assets authored with negative or greater-than-1 morph weights are still animated correctly, but the slider caps the manual range. Manual poses are session-only and are not written back to the file. While a clip is playing, its morph tracks own the influences and the sliders follow along; pause to pose by hand.
