...
- **Transform nodes** — select any node in the hierarchy (a mesh, a group, or the top-level row for a whole imported file) to attach a gizmo. **Move / Rotate / Scale** buttons in the Selection panel, or `G` / `R` / `S`, pick the gizmo mode; **Local / World** switches whose axes it follows. The inspector's **Position**, **Rotation** (degrees, in the node's Euler order), and **Scale** fields are editable and track the gizmo live. `Cmd/Ctrl+Z` / `Cmd/Ctrl+Shift+Z` share one undo history with bone posing, and **Reset Transform** returns the node to its load-time values. Transforms are session-only and are not written back to the file; changing the up axis re-seats the whole scene on the grid as before.
...
- Node transforms (move / rotate / scale) and bone poses are session-only viewer state: nothing is written back to the file, and re-opening the file starts from the authored transforms. There is no snapping, pivot editing, or multi-select.
