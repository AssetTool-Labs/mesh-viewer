...
- **Texture browser** — the Textures tab previews each texture over a checkerboard, with an optional UV-map overlay and metadata (size, wrap, filter, color space, and which materials use it). **Click a preview to enlarge it** in a modal for close inspection (`Esc` or click to dismiss). Compressed **KTX2/Basis** textures preview too — being GPU-only, they're rendered through a render-target read-back rather than a direct draw.
...
- **Inspect modes** — the **Inspect** group at the bottom of the Shading dropdown shows one PBR channel *unlit* on the geometry: **Base Color**, **Normal Map**, **Metalness**, **Roughness**, **AO**, or **Emissive** (alongside the existing **Normals** debug view). Each shows that slot's texture, or the material's scalar factor as a flat color when there is no map, so a swapped, missing, or mis-authored map is obvious at a glance. Packed maps (e.g. a single ORM texture) show all their components. This is the on-geometry counterpart of the **Textures** tab, which previews the 2D images — its **Isolate on model** button jumps straight to the Inspect mode for the selected texture. Inspect modes are remembered with the other view settings; the texture-map toggles apply only in Material/Rendered.
- **Viewport status chip** — whenever the viewport shows a special view — an Inspect channel, or the skin-weight display (which overrides the shading dropdown on skinned meshes) — a chip under the shading strip names it (e.g. *Inspect: Roughness*, *Skin weights: Isolate bone*) with a ✕ that returns to the previous shading mode or turns weights off. Skin weights take precedence when both are active.
