...
- **Transform nodes** — select any node in the hierarchy (a mesh, a group, or the top-level row for a whole imported file) to attach a gizmo. A Blender-style toolbar on the viewport's left edge holds **Move / Rotate / Scale** (or press `G` / `R` / `S`), the **Local / World** orientation toggle (Blender's *Local* / *Global*: the gizmo's axes follow the node's own frame or the scene's), **Undo / Redo**, and **Save selection as…**. As in Blender, an arrow moves along one axis, a square across two, and the center handle moves freely in screen space. The inspector's **Position**, **Rotation** (degrees, in the node's Euler order), and **Scale** fields are editable and track the gizmo live. `Cmd/Ctrl+Z` / `Cmd/Ctrl+Shift+Z` share one undo history with bone posing, and **Reset Transform** returns the node to its load-time values. Transforms are session-only and are not written back to the opened file (use *Save selection as…* to keep them); changing the up axis re-seats the whole scene on the grid as before.
- **Save selection as a new file** — the toolbar's save button (also in the Selection panel) writes the selected node and everything under it, with its current transform and pose baked in, to a new file. Select a file's top-level row to save that whole model. The dialog defaults to the source format when three.js can write it, and to GLB otherwise; the extension you pick decides the format:
...
- Node transforms (move / rotate / scale) and bone poses are session-only viewer state: nothing is written back to the opened file, and re-opening it starts from the authored transforms. *Save selection as…* writes a new file instead. There is no snapping, pivot editing, or multi-select.
- *Save selection as…* writes the node relative to its file's own frame: edits on the node and its parents are baked in, the viewer's up-axis display rotation and grid seating are not. A skinned mesh saved without its skeleton (select the file's top-level row to include it) comes out unskinned. Hidden nodes are skipped. Compressed KTX2 textures are decoded through the GPU before being embedded as PNG, so they grow.
