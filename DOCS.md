...
- *Export selection as…* writes the node relative to its file's own frame, and *Export scene as…* writes every file relative to the scene: edits on the nodes and their parents are baked in, the viewer's up-axis display rotation and grid seating are not. A skinned mesh saved without its skeleton (select the file's top-level row to include it) comes out unskinned. Hidden nodes are skipped. Compressed KTX2 textures are decoded through the GPU before being embedded as PNG, so they grow.
