...
- **Measurement** — a map-style **scale bar** in the bottom-left shows the current world scale as you zoom. The **ruler** button in the shading HUD enters measure mode: click two points on the model to get the straight-line distance, drawn as an always-on-top segment with a label at its midpoint; a third click starts a new segment, and `Esc` (or clicking the button again) exits. Units are asset-dependent, so the raw distance is shown (glTF is meters). Per-node and whole-scene bounding-box dimensions also appear in the Selection inspector and the Info tab.
