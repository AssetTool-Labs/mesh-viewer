...
- **Shading modes** — smooth, flat, wireframe, points (vertices as screen-space dots), and a normals debug shader.
- **Shading options** — a foldable **Shading options** group in the View tab holds controls specific to the current shading mode (and hides when the mode has none). In **Material/Rendered** it's the **Texture maps** checkboxes that switch each PBR map on or off *in the lit render* — **Base Color**, **Normal**, **Metalness**, **Roughness**, **AO**, **Emissive** — so you can drop the normal map to judge geometry, or AO to see it unshadowed, in any combination. In **Points** it's a **point size** slider; in **Wireframe** a **wireframe color** picker. This is distinct from the **Textures** tab, which previews the individual 2D images.
