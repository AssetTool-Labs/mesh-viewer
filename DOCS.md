...
- **Snapshot** — the camera button in the shading HUD (or the *3D Mesh Viewer: Save Snapshot as PNG* command) exports the current view to a PNG via a Save dialog. The scene is captured as shown — grid and axes are included if visible, the corner nav gizmo never is — so toggle overlays off first for a clean shot, or set the background color you want.
